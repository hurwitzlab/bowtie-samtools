...
bash run.sh ${GENOME_DIR} ${BT2_IDX} -1 ${M1} -2 ${M2} -U ${UNPAIRED} ${INPUT_FMT} ${OUT_DIR} ${BAM_NAME} ${LOGFILE} ${ALIGNMENT_TYPE} ${END_TO_END_PRESETS} ${LOCAL_PRESETS} ${NON_DETERMINISTIC} ${TRIM5} ${TRIM3} ${MINFRAGLEN} ${MAXFRAGLEN} ${THREADS} ${ADDITIONAL}
